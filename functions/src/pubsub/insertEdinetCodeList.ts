...
import {
  EdinetCode,
  EdinetCodeCnverter,
} from "../util/common/lib/firestore/models/edinetCode";
...
export const insertEdinetCodeList = functions
  ...
  .pubsub.topic("topic-insertEdinetCodeList")
  .onPublish(async (event) => {
    const promises = (edinetcodeDlInfo as any[])
      ...
      .map((info) => {
        ...
        // TODO: どっかにconveterをまとめる
        const converter: admin.firestore.FirestoreDataConverter<EdinetCode> =
          EdinetCodeCnverter;
        return db
          .collection("edinet_codes")
          .doc(hash)
